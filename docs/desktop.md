...
- **通知中心**：点时钟或 Cmd+Shift+N 打开；上面月历，下面历史、勿扰开关、全部清除；往旁边滑删除。
  键盘：j k / ↑↓ 选通知（第一次按只显示焦点框），Enter 执行，x / Delete 删除，Shift+Delete 全部清除，d 勿扰，h l / ←→ 翻月，. 回到本月，Esc 关闭
- **勿扰**：只有紧急通知还会弹出
...
键盘背光（Touch Bar 按住 fn 的背光键）显示 󰌌 和进度；大写锁定（同时按左右 Shift）显示「大写锁定 开 / 关」。
